...
public:
	...
	void simulationStep() {
		...
		for (Agent& agent : agents) {
			for (int good = 0; good < numGoods; good++) {
				float labour = agent.labour_allocations[good];
		...
		for (Agent& agent : agents) {
			...
			for (int good = 0; good < numGoods; good++) {
				...
				values.push_back(dVdL);
			}
			int index_most_val = max_element(values.begin(), values.end()) - values.begin();
			agent.labour_allocations[index_most_val] *= (1.0f + increment_rate);
			float sum = 0.0f;
...
int main() {
	{
		EconomicSimulation econSim(10, 10);
		econSim.simulationStep();
		cout << "wage rate: $" << econSim.agents[0].wage_rate << "\n";
	}
	cout << "Hello Girls!";
	while (true) {}
	return 0;
